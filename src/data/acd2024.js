import aditiSawhneyImg from "../assets/speakers/Aditi Sawhney.jpg";
import adityaSoniImg from "../assets/speakers/Aditya Soni.jpg";
import amitKumarImg from "../assets/speakers/Amit Kumar.jpg";
import dipaliKulshresthaImg from "../assets/speakers/Dipali Kulshrestha.jpg";
import dimpleVaghelaImg from "../assets/speakers/Dimple Vaghela.jpg";
import dishaBablaImg from "../assets/speakers/Disha Babla.jpg";
import drAbhilashaVyasImg from "../assets/speakers/Dr. Abhilasha Rakesh Vyas.jpg";
import faizalKhanImg from "../assets/speakers/Faizal Khan.jpg";
import nileshVaghelaImg from "../assets/speakers/Nilesh Vaghela.jpg";
import poojaRanjanImg from "../assets/speakers/Pooja Ranjan.jpg";
import rajatAroraImg from "../assets/speakers/Rajat Arora.jpeg";
import ridhimaKapoorImg from "../assets/speakers/Ridhima Kapoor.jpg";
import ruchiSoniImg from "../assets/speakers/Ruchi Soni.jpeg";
import srushithRepakulaImg from "../assets/speakers/Srushith Repakula.jpg";
import stephenHowellImg from "../assets/speakers/Stephen Howell.jpg";
import tejasGuptaImg from "../assets/speakers/Tejas Gupta.jpg";
import toshalKhawaleImg from "../assets/speakers/Toshal Khawale.jpg";
import awsLogo from "../assets/sponsors/aws.png";
import devClubLogo from "../assets/sponsors/Dev Club.jpg";
import snowflakeLogo from "../assets/sponsors/SnowFlake.jpg";
import image2 from "../assets/events/ACD 2024/image2.jpg";
import image3 from "../assets/events/ACD 2024/image3.jpg";
import image6 from "../assets/events/ACD 2024/image6.jpg";
import image8 from "../assets/events/ACD 2024/image8.jpg";
import image11 from "../assets/events/ACD 2024/image11.jpg";
import image12 from "../assets/events/ACD 2024/image12.jpg";
import image14 from "../assets/events/ACD 2024/image14.jpg";
import image15 from "../assets/events/ACD 2024/image15.jpg";
import image16 from "../assets/events/ACD 2024/image16.jpg";
import image17 from "../assets/events/ACD 2024/image17.jpg";
import image18 from "../assets/events/ACD 2024/image18.jpg";
import image19 from "../assets/events/ACD 2024/image19.jpg";
import image20 from "../assets/events/ACD 2024/image20.jpg";
import image21 from "../assets/events/ACD 2024/image21.jpg";
import image22 from "../assets/events/ACD 2024/image22.jpg";

// ACD 2024 Event Data (Past Event)
export const acd2024Data = {
  // Hero Section Data
  title: "AWS Community Day Delhi 2024",
  date: "September 21, 2024",
  venue: "IIT Delhi, Hauz Khas, New Delhi",
  description: [
    "AWS Community Day Delhi 2024 was a day-long, community-driven event that celebrated learning, collaboration, and innovation with AWS technologies.",
    "Cloud enthusiasts, developers, students, IT professionals, and business leaders came together for an extraordinary experience of knowledge sharing and networking.",
  ],

  // Images for marquee
  images: {
    imagePaths: [
      image2,
      image3,
      image6,
      image8,
      image11,
      image12,
      image14,
      image15,
      image16,
      image17,
      image18,
      image19,
      image20,
      image21,
      image22,
    ],
  },

  // Event Details
  eventDetails: {
    type: "Community Day",
    organizer: "AWS User Group Delhi NCR",
    attendees: "350+",
    sessions: "10",
    speakers: "17",
    status: "completed",
  },

  // About sections for the event
  aboutSections: [
    {
      title: "WHO ATTENDED",
      content: [
        "AWS Community Day Delhi 2024 brought together developers, architects, cloud enthusiasts, IT professionals, students, and business leaders from diverse backgrounds.",
        "The event catered to anyone passionate about AWS technologies, whether beginners or seasoned experts.",
        "Over 500 attendees connected, shared knowledge, and built lasting relationships within the AWS community.",
      ],
    },
    {
      title: "THE EVENT",
      content: [
        "This day-long event was filled with immersive experiences, from hands-on workshops to deep-dive technical sessions.",
        "Attendees explored the latest trends and innovative implementations guided by AWS veterans, experts, and industry leaders.",
        "It truly reflected the community spirit — created by the community, for the community — delivering highly relevant and engaging content.",
      ],
    },
    {
      title: "WHY IT MATTERED",
      content: [
        "The event reignited passion for cloud technology and helped accelerate AWS journeys for many attendees.",
        "With practical insights, real-world use cases, and success stories, participants left equipped with knowledge they could directly apply in their businesses or careers.",
        "From networking opportunities to exclusive AWS goodies, the event offered memorable professional and personal takeaways.",
      ],
    },
  ],

  // Speakers data for ACD 2024
  speakers: [
    {
      name: "Aditi Sawhney",
      image: aditiSawhneyImg,
      linkedin: "https://www.linkedin.com/in/sawhneyaditi/",
    },
    {
      name: "Aditya Soni",
      image: adityaSoniImg,
      linkedin: "https://www.linkedin.com/in/adityasonittyl/",
    },
    {
      name: "Amit Kumar",
      image: amitKumarImg,
      linkedin: "https://www.linkedin.com/in/amitkyvmw/",
    },
    {
      name: "Dipali Kulshrestha",
      image: dipaliKulshresthaImg,
      linkedin: "https://www.linkedin.com/in/dipalik/",
    },
    {
      name: "Dimple Vaghela",
      image: dimpleVaghelaImg,
      linkedin: "https://www.linkedin.com/in/dimple-vaghela-ba45447b/",
    },
    {
      name: "Disha Babla",
      image: dishaBablaImg,
      linkedin: "https://www.linkedin.com/in/dishababla/",
    },
    {
      name: "Dr. Abhilasha Rakesh Vyas",
      image: drAbhilashaVyasImg,
      linkedin: "https://www.linkedin.com/in/abhilasharvyas/",
    },
    {
      name: "Faizal Khan",
      image: faizalKhanImg,
      linkedin: "https://www.linkedin.com/in/faizal-khan/",
    },
    {
      name: "Nilesh Vaghela",
      image: nileshVaghelaImg,
      linkedin: "https://www.linkedin.com/in/nilesh-vaghela/",
    },
    {
      name: "Pooja Ranjan",
      image: poojaRanjanImg,
      linkedin: "https://www.linkedin.com/in/poojaranjan22/",
    },
    {
      name: "Rajat Arora",
      image: rajatAroraImg,
      linkedin: "https://www.linkedin.com/in/arora-rajat-cw/",
    },
    {
      name: "Ridhima Kapoor",
      image: ridhimaKapoorImg,
      linkedin: "https://www.linkedin.com/in/kapoor-ridhima/",
    },
    {
      name: "Ruchi Soni",
      image: ruchiSoniImg,
      linkedin: "https://www.linkedin.com/in/ruchi-soni-78396715/",
    },
    {
      name: "Srushith Repakula",
      image: srushithRepakulaImg,
      linkedin: "https://linkedin.com/in/srushithrepakula",
    },
    {
      name: "Stephen Howell",
      image: stephenHowellImg,
      linkedin: "https://www.linkedin.com/in/stephenrichardhowell/",
    },
    {
      name: "Tejas Gupta",
      image: tejasGuptaImg,
      linkedin: "https://www.linkedin.com/in/tejasguptaa/",
    },
    {
      name: "Toshal Khawale",
      image: toshalKhawaleImg,
      linkedin: "https://www.linkedin.com/in/toshal-khawale/",
    },
  ],

  // Event highlight words for marquee
  highlightWords: [
    "INNOVATION",
    "COMMUNITY",
    "CLOUD",
    "NETWORKING",
    "LEARNING",
  ],

  // Event sponsors
  sponsors: {
    poweredBy: [
      {
        name: "AWS",
        logo: awsLogo,
        website: "https://aws.amazon.com",
      },
    ],
    inAssociationWith: [
      {
        name: "Dev Club",
        logo: devClubLogo,
        website: "https://www.instagram.com/devclub_iitd/",
      },
    ],
    platinum: [],
    gold: [],
    silver: [
      {
        name: "Snowflake",
        logo: snowflakeLogo,
        website: "https://snowflake.com",
      },
    ],
  },

  // Looking Ahead / Community Section
  community: {
    heading: "Looking Ahead",
    subheading:
      "This was just the beginning — join our community to get early access and invites for future events.",
    description:
      "Connect with fellow attendees, get event announcements, and be the first to pre-register for upcoming AWS Community Days.",
    meetupUrl: "https://www.meetup.com/amazonaws-delhi/",
  },
};

export default acd2024Data;
